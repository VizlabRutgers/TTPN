...
void stringtokenizer( vector<string> & OutputStringVector,  const  string  & InputString,  const  string  & lookfor );
...
void CreateNodeConn_Array(int** node_conn_array,int z_dim,int y_dim, int x_dim,int numberOfPointsInCell,int ncells);
void CreateCoord_Array(vector<float> &allvariables,vector<float> &xdimension, vector<float> &ydimension, vector<float> &zdimension,
...
void parseConfigFile(string &Datapath, string &FileBaseName,string &FileExtention,vector<string> &variableNamesvect,string Configfilename, int &InitialtimeStep, int &FinaltimeStep, float &deltaxval, float &deltayval, float &deltazval,int &SmallestObjVol, int &TimePrecision, int &TimeIncrement, float &thresh1, float &thresh2);
...
void findVariablesintheAttributeFiles1(ThePetriNet &CurrentPetriNet, string file,string fileExtention, int currentFrame, int Frame, int AttributeNumber, int ObjectID, double &VariableValue, string & StrVersionofVariable);
void findVariablesintheAttributeFiles2(ThePetriNet &CurrentPetriNet, int currentFrame,int Frame, int AttributeNumber, int ObjectID, double &VariableValue, string & StrVersionofVariable,int transitionOrPlace, long currentDurationinthePlace);
...
void readPT_ConfFile(string &filename,PNAllVaribles & currentAllVaribles,vector<Arc> &inputArcs, vector<Arc> &outputArcs);
void readPT_ConfFile0(string &filename, int &PNumber, int &transitionNumber,vector<Arc> &inputArcs, vector<Arc> &outputArcs, vector<int> &FinalPlaces, vector<string> &transitionVector,vector<string> &placeVector,vector<int> &InitialPlaces,vector<string> &PetriNetVariables);
void readPT_ConfFile2(string &filename, int &PNumber, int &transitionNumber,vector<Arc> &inputArcs, vector<Arc> &outputArcs, vector<int> &FinalPlaces, vector<string> &transitionVector,vector<string> &placeVector,vector<int> &InitialPlaces,vector<string> &PetriNetVariables);
int checkGreater(ThePetriNet &CurrentPetriNet, int PlaceOrTransition,int location, float currentValue);
void createPT2(ThePetriNet &CurrentPetriNet, int PNumber, int transitionNumber,vector<Arc> &inputArcs, vector<Arc> &outputArcs,vector<int> &finalPlaces,vector<string> &transitionVector,vector<string> &placeVector,vector<int> &initialPlaces,vector<int> &AttributeNumbers);
void createPT(ThePetriNet &CurrentPetriNet, vector<Arc> &inputArcs, vector<Arc> &outputArcs);
int checkTransitionFired(ThePetriNet &CurrentPetriNet, int TransitionID, stObject curDummyObj,int currentFrame);
void updateObjects2(ThePetriNet &CurrentPetriNet, vector<stObject> &currentObjects, int currentFrame,vector<int> &initialPlaces, vector<int> &finalPlaces, int InitialtimeStep, vector<string> &trakTable,string trakTableFile );
void evaluatePT(ThePetriNet &CurrentPetriNet,vector<Arc> &inputArcs, vector<Arc> &outputArcs,vector<int> &finalPlaces, vector<float> &ActionVector,int currentFrame,int InitialtimeStep,int FinaltimeStep);
void evaluatePT2(ThePetriNet &CurrentPetriNet,vector<Arc> &inputArcs, vector<Arc> &outputArcs,vector<int> &finalPlaces, vector<float> &ActionVector,int currentFrame,int InitialtimeStep,int FinaltimeStep);
void evaluatePT3(ThePetriNet &CurrentPetriNet,vector<Arc> &inputArcs, vector<Arc> &outputArcs,vector<int> &finalPlaces, vector<float> &ActionVector,int currentFrame,int InitialtimeStep,int FinaltimeStep);
void evaluatePT4(ThePetriNet &CurrentPetriNet,vector<Arc> &inputArcs, vector<Arc> &outputArcs,vector<int> &finalPlaces,int currentFrame,int InitialtimeStep,int FinaltimeStep);
void evaluatePT5(ThePetriNet &CurrentPetriNet,vector<Arc> &inputArcs, vector<Arc> &outputArcs,vector<int> &finalPlaces,int currentFrame,int InitialtimeStep,int FinaltimeStep);
void evaluatePT6(ThePetriNet &CurrentPetriNet,vector<Arc> &inputArcs, vector<Arc> &outputArcs,vector<int> &finalPlaces,int currentFrame,int InitialtimeStep,int FinaltimeStep);
            ...
            for(int dummyarc = 0; dummyarc <  CurrentPetriNet.CurrentTransitions[dummytransition].incomingArcs.size();dummyarc++)
            {
                ...
                for(int dumObjIndex = 0; dumObjIndex < CurrentPetriNet.CurrentPlaces[ dummyPlaceID - 1 ].currentObjectsInThisPlace.size();dumObjIndex++)
                {
                    ...
                    if (curDummyObj.activeObject == 1)
                    {
                        ...
                        transitionfires = checkTransitionFired( CurrentPetriNet, dummytransition+1, curDummyObj,currentFrame );
                        if(curDummyObj.RealObjectID == 22 && transitionfires == 1)
                            curDummyObj.RealObjectID = 22;
                        transitionfires = checkTransitionFired( CurrentPetriNet, dummytransition+1, curDummyObj,currentFrame );
...
//        if ((CurrentPetriNet.CurrentPlaces[dummyplaceID].FinalPlace == 1) ||(CurrentPetriNet.CurrentPlaces[dummyplaceID].PlaceID == 2) ||(CurrentPetriNet.CurrentPlaces[dummyplaceID].PlaceID == 3))
    {
      int dummyvar = 0;
      for (; dummyvar < CurrentPetriNet.CurrentPlaces[dummyplaceID].currentObjectsInThisPlace.size() ; dummyvar++ )
      {
        int dummyplacee = -1;
